...
<head>
	...
	<script type="text/javascript" charset="utf-8">
		...
		function PhoneListControllerAjax($scope, $http){
			$http({
				method: 'POST', //방식
				url: '01.json', /* 통신할 URL */
...
<body>
	...
	<ul ng-controller="PhoneListController">
		<li ng-repeat="phone in phones1 | filter:searchKeyword | orderBy:orderProperty">
			번호 : {{phone.no || '없음'}}<br />
			이름 : {{phone.name || '없음'}}<br />
			설명 : {{phone.snippet || '없음'}}
		</li>
	...
	<p>
		...
		현재 정렬 기준 : {{orderProperty || '없음' }}
	</p>
	
	----------------------
	<ul ng-controller="PhoneListControllerAjax">
		<li ng-repeat="phone in phones2">
			번호 : {{phone.no}}<br />
			이름 : {{phone.name}}<br />
			설명 : {{phone.snippet}}
		</li>
